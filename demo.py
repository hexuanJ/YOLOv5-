...
class PPE_detect:
    ...
    def __init__(self):
        ...
        weights_path = os.path.join(project_dir, 'weights', 'ppe_yolo_n.pt')
        self.model = torch.hub.load('./yolov5', 'custom', path=weights_path, source='local', force_reload=True)  # local repo
        # 置信度阈值
        self.model.conf = 0.3
        # 加载摄像头
